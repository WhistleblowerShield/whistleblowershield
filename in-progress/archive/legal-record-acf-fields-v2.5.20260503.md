# Legal Record ACF Canonical Field Draft (v2.5)
...
- Defined `[+]` annotation in Slug-to-Companion Map legend (was undefined).
- Added boolean conditional forms to accepted-annotation list.
- Enumerated 4 plain-english `ws_auto_` stamps explicitly.
- Added worked examples for tab keys containing `&` and `/`.
- Added `comlaw` ↔ `jx-common-law` post-type mapping note.
- Defined `mandate_date`.
- Relocated `court_is_fed` from Identity Tab to Hidden Fields as `_court_is_fed`.
- Added inline conditional annotation on `burden_shifting_details`.
- Fixed slug-map recognition-vocab annotations: `malicious-reporting-sanctions` (`Applies` → `Specified`); added `Specified` to `process-pathway`.
- Added nested-cluster note to `protected-action` slug-map line for the `reasonable-belief` branch.
- Added new section: **Cross-Taxonomy Slug-to-Companion Map** (companions triggered by terms outside `ws_legal_recognition`).
- Added eligible-taxonomy allowlist for `extended_taxonomies` / `suppressed_taxonomies`.
- Restructured Substantive-Record Common Fields header into "Excluded From Substantive" and "Substantive Additions" subsections.
- Promoted freetext default to a top-of-section rule in [Default Field Types]; field shape never inferred from a name's apparent meaning, only from explicit convention or inline definition.
- Annotated `enforcement_sequence` shape as freetext per default rule; pending repeater-vs-freetext decision (see Notes).
- Promoted `federal_agencies`, `_primary_agency_is_fed`, and `_related_agencies` from Statute-Specific / Precedent-Record-Common deltas to Common Fields.
- Renamed precedent `types` (multi-select, Citation-Specific) to `class` (single-select, Precedent-Record Common); added `has-details` choice with `class_details` companion. Removed Construction-Specific `type` (now covered by common `class`). Removed Citation-Specific `type_context`.
- Renamed `Source / Audit Tab` → `Authority Tab` (everywhere); moved `last_reviewed_date` to end of Identity Tab with annotation "must be last after any insertions"; updated prompt-schema map (`source_audit.url` → `authority.url`); updated precedent insertion (`#### Source / Audit Tab` → `#### Authority Tab`).
- Renamed slug `sovereign-immunity-waiver` → `blanket-sovereign-immunity-waiver` and documented exclusion of `sovereign-immunity-status` slug.
- Renamed field `sovereign_immunity_waiver` → `sovereign_immunity_waiver_class` to disambiguate from the slug.
- Replaced slug-map `[C]` annotation with `[+]` to resolve collision with the hooks-doc `[C]` (Cross-Field Invalid Combos), which now stays unique.
- Restored `### Statute-Specific` and `### Citation-Specific` blocks as structural slots with explicit "no deltas from common records" notes (rather than placeholders).
- Added [Ingest Pipeline Philosophy] subsection to Notes — codifies researcher → reconciler → human staging and frames freetext and `*_context` fields as designed breadcrumb channels, not failure modes.
...
Umbrella values in multi-select fields and selectable multi-taxonomy fields must end with `-only`.

Use `-only` only when the value represents a blanket selection that excludes granular sibling values in the same
field. Do not add `-only` to ordinary granular values.

Hooks should target the `-only` value and clear or reject granular sibling values in the same field. Sentinels such
as `has-details` and `see-context` are not granular values and may remain valid with an umbrella value when the
field's companion logic permits them.
...
CPT slot values map to registered post types as follows:

- `statute`      → `jx-statute`
- `comlaw`       → `jx-common-law`
...
Worked examples for tab labels containing `&` or `/`:

- `Statute of Limitations & Thresholds` → `statute_of_limitations_thresholds_tab`
- `Processes & Remedies`                → `processes_remedies_tab`
- `Waiver & Scope`                      → `waiver_scope_tab`
- `Source / Audit`                      → `source_audit_tab` (renamed to `Authority` → `authority_tab` in v2.5)

The `&` and `/` characters are dropped; adjacent words join with a single underscore.
...
freetext field. Field shape is never inferred from the field name's apparent meaning — only from explicit
convention or explicit inline definition.
...
When using `AND` or `OR`, repeat the full condition on both sides rather than relying on implied trigger fields.
...
- `enforcement_sequence`           — (freetext per default; describes priority of enforcement agencies and any
                                      enforcement requirements; see Notes for repeater-vs-freetext discussion
                                      flagged in v2.5)
...
- `sovereign_immunity_waiver_class` — (conditional on `sovereign_immunity_status` is NOT `not-waived`; select:
                                      `explicit-waiver`|`implied-waiver`; renamed from `sovereign_immunity_waiver`
                                      in v2.5 to disambiguate from the `blanket-sovereign-immunity-waiver` slug)
...
The following precedent-only fields capture modifications to legal definitions and do not appear on substantive records:
...
- `class`                          — (select: `case-law`|`statute`|`regulatory`|`secondary`|`has-details`;
                                      single-select; renamed from Citation-Specific `types` and promoted to
                                      Precedent-Record Common in v2.5)
...
- `_court_is_fed`                  — (derived from `court` `ws_jx_codes`; relocated from Identity Tab in v2.5)
...
This draft uses the statute field set as the baseline for broad legal-record parity, then adds per-type deltas
...
This spec has zero live-data implications. It is a structural draft for PHP field generation and ingest schema
mapping.
