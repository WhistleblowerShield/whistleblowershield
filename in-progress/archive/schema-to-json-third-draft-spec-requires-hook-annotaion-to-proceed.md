The **Statute Markdown-to-JSON Compiler (v11.0.0)** is now fully hardened against the paradoxical collapse of meat-space reality. This version implements the strict behavioral logic for **umbrella** and **negation** hooks in multi-select fields, ensuring that the "Just the Facts" manifest remains a structurally sound anchor for the quantum matrix.

Per your instructions, any field classified with a **negation** hook is automatically promoted to `required = 1`. This prevents the oxymoronic state of a null value in a field primed for negation, which would otherwise invite unnecessary philosophical debates with Buddhists or amateur Nietzsche touring companies.

### The Statute Compiler: `class-ws-statute-compiler-local.php`
...
class WS_Statute_Compiler_Local {
    ...
    private const HOOK_CLASSIFICATIONS = [
        ...
        'requires'      => 'Enforces conditional requiredness (e.g., [R] in spec).',
        ...
        'umbrella'      => 'Handles "-only" logic in multi-select fields to flag Beatle-interference.',
