...
3. **Manual data build — California** — full depth: statutes, citations,
   constructions, agencies, procedures, assist orgs. California is the
   stress test — complex, well-documented, lots of case law. Taxonomy
   completeness audit required before cascade testing begins.

4. **Manual data build — thin jurisdiction (Wyoming)** — genuinely thin
   legal landscape. Proves the fallback sequence works and that the
   platform handles incompleteness honestly. Wyoming data set:
   - 3 statutes: `WY-9-11-103`, `WY-27-11-109`, `WY-42-4-304`
   - 1 common law doctrine: `WY-CL-PUBLIC-POLICY` (Wrongful Discharge
     in Violation of Public Policy — private sector primary protection)
   Wyoming's common law record is critical to the fallback demo —
   it demonstrates that the platform represents the actual legal
   landscape honestly, including judicially-recognized protections
   that exist outside codified statute.

5. **`render-common-law.php` implementation** — currently a stub.
   Must be implemented before the Wyoming jurisdiction page can
   surface common law records. See render README for implementation notes.

6. **`render-assist-org.php` extraction** — assist org rendering must be
   extracted from `render-section.php` into its own file before Phase 2.
   Assist org cards render differently in the directory context than on
   a jurisdiction page. The Phase 2 filter cascade needs to call them
   independently. This is a pre-Phase 2 prerequisite, not an optional
   refactor.

7. **Build `tool-generate-prompt.php`** — prerequisite for production
   data runs. Build after demo data is complete so you have a validated
   dataset to test against.

8. **Build `tool-ingest.php`** — same reasoning. Manual population for
...
### Common law records in the filtered path

`jx-common-law` records participate in the filtered path the same way
`jx-statute` records do — via taxonomy match, `attach_flag` ignored.
`ws_get_jx_common_law_data()` is the query function. The filter cascade
must query both CPTs and surface common law results in their own section,
distinct from statutes, with appropriate framing.

The `ws_cl_statutory_preclusion` flag is relevant to the filtered path:
when `true`, the platform should note that the common law claim may be
unavailable if a statutory remedy applies — surface this as a user-facing
notice, not a filter exclusion. The record is still shown; the preclusion
is a clarification.
...
→ Results:
   ...
   Priority 1b — Relevant common law protections (where applicable)
...
|---|---|
| Understand my rights | Statutes → common law → procedures → assist orgs |
| File a complaint | Procedures (deadline-bearing first) → agencies → statutes |
| Find legal help | Assist orgs → procedures → statutes |
| All of the above / not sure | Statutes → common law → procedures → agencies → assist orgs |
...
landscape — not arithmetic. Wyoming's threshold accounts for its
thin statutory landscape — the common law record supplements but
does not change its genuinely limited coverage for most situations.
...
Zero results: same behavior. The user still gets the fallback resources.
...
3 statutes + 1 common law doctrine (`WY-CL-PUBLIC-POLICY`).
Proves the fallback sequence reads as honest and helpful rather
than broken. The common law record demonstrates the platform
surfaces the full picture of what a jurisdiction actually offers —
including judicially-recognized protections outside codified statute.
The statutory preclusion notice on the common law record is a concrete
example of editorial honesty: "this protection may not be available
...
3. Thin jurisdiction (Wyoming) → fallback sequence + common law notice
...
and common law notice with real lay users.
...
- Labor: Fixed monthly stipend equivalent to California minimum wage
  ($17.28/hour) × 28 hours/week (~$1,935/month). This represents a
  modest baseline acknowledgment of ongoing work, not a cap on hours
  invested or a guarantee of compensation.
...
Transparent expense ledger. Modest retro stipend.
...
Every statute, citation, construction, agency, procedure, and assist
org audited for taxonomy coverage on every relevant axis. Gaps documented
explicitly rather than left as silent omissions.

**Pre-cascade checklist for Wyoming data:**
All 3 statutes and the `WY-CL-PUBLIC-POLICY` common law record audited
for taxonomy completeness. The common law record's `ws_cl_statutory_preclusion`
flag verified correct (true for Wyoming). `ws_cl_public_policy_sources`
confirmed (constitution, statute, administrative-rule per Allen v. Safeway
and McLean v. Hyland precedent).
