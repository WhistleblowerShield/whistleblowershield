...
### Second fine-tooth-comb pass: query-agencies.php + query-helpers.php

Follow-up scan of the remaining query-layer siblings (`query-agencies.php`,
`query-directory.php`, `query-general.php`, `query-shared.php`,
`query-helpers.php`), prompted by the arity-drift bug class found above.
`query-directory.php`, `query-general.php`, and `query-shared.php` reviewed
clean — no changes. Two real findings, both fixed:

- **`query-agencies.php`** — the exact same `is_wp_error()`-vs-empty
  conflation already fixed for `jx_terms`/`disc_types` in this same file
  (Stage 3, 3.20.1) was missed for `ws_procedure_type` resolution, in two
  places: `ws_build_agency_procedure_row()` and
  `ws_get_procedures_for_record()`. `ws_procedure_type` decides which group
  (disclosure/retaliation/both) a procedure card renders under — a genuine
  query failure previously produced the same `''` as "type genuinely not
  set yet," silently mis-sorting or dropping the card with no record of
  why. Both now log the WP_Error case, matching the established pattern in
  the same file. Also fixed two stale comments: a changelog reference to
  the old function name `ws_get_procedures_for_parent()` (renamed to
  `ws_get_procedures_for_record()` at some point, comment never updated),
  and a return-shape doc line that said `jurisdiction` (singular) when the
  actual returned key is `jurisdictions` (plural).
- **`query-helpers.php`** — `ws_court_lookup()`'s own docblock documented
  the pre-fix admin-only limitation ("on the frontend these globals are
  empty and this function always returns null") as expected, unremarkable
  behavior rather than the bug it was. Updated to describe the actual
  current (fixed) state, with a note on what it used to say and why.

---

## Status: what's left

Nothing known. Stages 0 through 5 are all complete, both fine-tooth-comb
passes' findings are fixed, and the pattern to keep applying to any new
code — loud, not silent, and verify the fail-loud mechanism is actually
loaded in whatever layer the new code runs in — is captured below for
reference.
