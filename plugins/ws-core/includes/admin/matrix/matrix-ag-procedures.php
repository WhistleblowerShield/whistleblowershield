 ...
 * PURPOSE
 * -------
 * Creates ag-procedure CPT posts for each agency's primary whistleblower
 * intake path. Each record links to its statute agency via ws_ag_procedure_agency_id,
...
<li><strong>File an award application when applicable.</strong> If the SEC brings an enforcement action resulting in sanctions exceeding $1 million and you believe your tip contributed, submit Form WB-APP to the SEC Whistleblower Office to claim your award (10–30% of collected sanctions).</li>
    ...
    [
        ...
        'deadline_days'        => 30,
        'deadline_clock_start' => 'adverse_action',
        'has_prerequisites'        => false,
        'prerequisites_details'   => '',
...
<li><strong>If OSHA takes no action within 180 days.</strong> Under several statutes (notably SOX and AIR21), you may file a new action in federal district court if OSHA has not issued a final order within 180 days of your complaint. Consult an attorney to evaluate whether to proceed administratively or judicially.</li>
        ...
        'exclusivity_details'     => 'Filing a complaint with OSHA under some statutes (e.g., SOX § 806) may restrict your ability to file in federal court while the OSHA proceeding is pending. Under other statutes you may withdraw from OSHA and proceed in court after 180 days. Review the specific statute governing your claim with an attorney before choosing your forum.',
    ],
...
<li><strong>After OSC&#8217;s determination.</strong> If OSC declines to pursue your complaint, it will issue a written determination. You then have 65 days from that determination to file an IRA appeal at the MSPB. Track this deadline precisely — missing it forfeits your right to independent adjudication.</li>
    ...
    [
        ...
        'deadline_days'        => 65,
        'deadline_clock_start' => 'knowledge',
...
<li><strong>Initial Decision and further appeal.</strong> After the hearing, the AJ issues an Initial Decision. Either party may petition the full MSPB Board for review within 35 days. After the Board&#8217;s final decision, you may petition the U.S. Court of Appeals for the Federal Circuit for judicial review.</li>
        ...
        'exclusivity_details'     => 'Filing an IRA appeal at the MSPB generally precludes pursuing the same claims in federal district court while the MSPB proceeding is pending. If the MSPB issues a final order, subsequent judicial review goes to the Federal Circuit, not a district court. Consult an attorney before choosing between the MSPB/Federal Circuit track and any available district court pathway.',
    ],
...
<li><strong>If a complaint is issued.</strong> The case proceeds to a hearing before an NLRB Administrative Law Judge. Remedies for retaliation may include reinstatement, back pay, and a notice posting. The Board&#8217;s order is enforced by the applicable U.S. Court of Appeals.</li>
        ...
        'exclusivity_details'     => 'Filing an ULP charge with the NLRB does not preclude filing claims under other federal statutes (e.g., SOX, FCA). However, remedies for the same underlying harm may overlap; consult an attorney to coordinate parallel claims and avoid double recovery arguments.',
    ],
...
<li><strong>Award determination.</strong> If CFTC sanctions exceed $1 million and CFTC finds your tip contributed, you may receive 10–30% of collected sanctions. Award determinations may take years after the enforcement action closes.</li>
        ...
        'exclusivity_details'     => 'Submitting a tip here does not constitute an anti-retaliation complaint. If you experienced adverse action for providing information to the CFTC, you may have a separate retaliation claim under CEA § 23(h)(1) in federal district court. That claim must be filed within 2 years of the retaliation.',
    ],
...
<li><strong>Award determination after collection.</strong> If the IRS collects additional taxes, penalties, and interest substantially based on your information, you will receive a preliminary award determination letter. You may contest the award amount by filing a petition in U.S. Tax Court within 30 days of the determination letter.</li>
        ...
        'exclusivity_details'     => 'The IRS Whistleblower program does not provide anti-retaliation protection. If you experience retaliation for reporting tax fraud, your civil remedy is a separate lawsuit in federal district court under IRC § 7623(d), which must be filed within 180 days of the adverse action.',
    ],
...
<li><strong>Federal court option after OSHA inaction.</strong> Under several environmental statutes, if OSHA has not issued a final order within 30 days of the complaint, you may file a new civil action in federal district court. Consult an attorney about the timing and forum selection for your specific statute.</li>
    ...
    [
        ...
        'entry_point' => 'mail',
        'intake_url'  => 'https://www.justice.gov/civil/fraud-section/contact-fraud-section',
        'identity_policy'      => 'identified',
        'intake_only'          => false,
        ...
        'deadline_clock_start' => '',
        'has_prerequisites'        => true,
        'prerequisites_details'   => 'You must retain an attorney before filing — the False Claims Act does not permit pro se qui tam relators. The complaint must be filed in federal district court under seal (not served on the defendant) and served simultaneously on the U.S. Department of Justice through the U.S. Attorney\'s Office for the filing district. Your attorney will also prepare a written disclosure statement containing all supporting information about the fraud.',
...
function ws_seed_procedure_matrix() {
    ...
    foreach ( $_ws_ag_procedure_matrix as $proc ) {

        // ── Resolve statute agency ─────────────────────────────────────────
        ...
        $existing = get_page_by_path( $proc['slug'], OBJECT, 'ag-procedure' );
        ...
        $meta = [
            ...
            'ws_ag_procedure_entry_point'             => $proc['entry_point'],
            'ws_ag_procedure_intake_url'              => $proc['intake_url'],
            'ws_ag_procedure_identity_policy'         => $proc['identity_policy'],
            'ws_ag_procedure_intake_only'             => $proc['intake_only'] ? 1 : 0,
            'ws_ag_procedure_deadline_days'           => (int) $proc['deadline_days'],
            'ws_ag_procedure_deadline_clock_start'    => $proc['deadline_clock_start'] ?? '',
            'ws_ag_procedure_has_prerequisites'           => $proc['has_prerequisites'] ? 1 : 0,
            'ws_ag_procedure_prerequisites_details'       => $proc['prerequisites_details'] ?? '',
            'ws_ag_procedure_walkthrough_wysiwyg'         => $proc['walkthrough_wysiwyg'] ?? '',
            'ws_ag_procedure_exclusivity_details'         => $proc['exclusivity_details'] ?? '',
        ];
        ...
        if ( ! empty( $proc['statute_slugs'] ) ) {
            $statute_ids = ws_ag_procedure_matrix_resolve_statute_ids( $proc['statute_slugs'] );
